	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			for (int i = 0; i < n; i++)
			{
				...
				// approximate average gap between primes less than or equal to current number (by the Prime Number Theorem)
				// approximate probability that current number is prime (by the Prime Number Theorem)
				// is current number prime?
				writer.write(nums[i] + " " + nth + " " + order + " " + df.format(orderProps[order]) 
					+ " " + (nums[i]-lastPrime) + " " + nums[i]%6 + " " + nums[i]%2 + " " + " " + Math.log(nums[i]) 
					+ " " + (1.0/(Math.log(nums[i]))) + " " + prime + "\n");
